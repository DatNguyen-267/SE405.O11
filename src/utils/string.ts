...
export function timeToString(time: number) {
  ...
  return `${date.getFullYear()}-${toString(date.getMonth() + 1)}-${toString(
    date.getDate(),
  )} ${toString(date.getHours())}:${toString(date.getMinutes())}:${toString(date.getSeconds())}`
}
...
export function formatFunctionSignatureName(inputText: string) {
  const words = inputText.split(/(?=[A-Z])/)
